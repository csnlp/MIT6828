...
boot_alloc(uint32_t n)
{
        ...
        // decide if there is enough nextfree. npages * PGSIZE is 
        // is all available space. 
...
page_init(void)
{
	...
	// (4). Extended memory: [EXTPHYSMEM, ...]
	// The followings are memory that being used in extended memory. 
	...
	// The followings are memory that are free in extended memory. 
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	for (int i=0; i < size; i+=PGSIZE) {
		// get the entry corresponding to va. 
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
	// Fill this function in
	pte_t *pte_ptr = NULL;
	pte_ptr = pgdir_walk(pgdir, va, 1);
	if (pte_ptr == NULL) {
		return -E_NO_MEM;
	}
	pp->pp_ref++;
	// If pp and va already are mapped. 
	if ((*pte_ptr) & (PTE_P)) {
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	...
	// find the va's corresponding page entry.
	pte_ptr = pgdir_walk(pgdir, va, 0);
...
page_remove(pde_t *pgdir, void *va)
{
	...
	// such pte doesn't exist
	if(page == NULL)
		return;
	/* if exits. 
	(1). decrease ref count 
	(2). free the physical page. 
	both (1) and (2) can be implemented with page_decref
	(3). the pg table entry corresponding to va should be set to 0 
	*/
	page_decref(page);
	tlb_invalidate(pgdir, va);
	*pte_ptr = 0;
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for(; start<end; start+=PGSIZE) {
		cur_pg = pgdir_walk(env->env_pgdir, (void *)start, 0);
		if((int) start > ULIM || cur_pg == NULL ||(((uint32_t)(*cur_pg) & perm) != perm)) {
